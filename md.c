...
// new vars
    ...
    double *av_x2,*av_y2,*av_z2;
    double d_x,d_y,d_z,range_x;
    int    meas_x,meas_y,meas_z,natoms1;
    double dL_max=0;
    double *sigma_x,*sigma_y,*sigma_z;
    double *sigma_x2,*sigma_y2,*sigma_z2;    
   ...
   struct stat *buf = malloc(sizeof(struct stat));   
    ...
    fclose(fp);
      ...
      tau_0 = round(100.0 / ((double)(cr->ms->sim+1)));
      
      if(tau_0 < 2) tau_0 = 2.0;
    ...
    snew(y_1,natoms1+1);
    snew(z_1,natoms1+1); 
    snew(corr_x,natoms1+1);
    snew(corr_y,natoms1+1);
    snew(corr_z,natoms1+1);     
    snew(dL_x,natoms1+1);
    snew(dL_y,natoms1+1);
    snew(dL_z,natoms1+1);         
    snew(av_x2,natoms1+1);
    snew(av_y2,natoms1+1);
    snew(av_z2,natoms1+1);     
    snew(sigma_x,natoms1+1);
    snew(sigma_y,natoms1+1);
    snew(sigma_z,natoms1+1); 
    snew(sigma_x2,natoms1+1);
    snew(sigma_y2,natoms1+1);
    snew(sigma_z2,natoms1+1); 
    
    av_x = (double **) malloc(sizeof(double)*(natoms1+1));
    ...
    av_y = (double **) malloc(sizeof(double)*(natoms1+1));
    ...
    av_z = (double **) malloc(sizeof(double)*(natoms1+1));
    ...
    while (!bLastStep || (bRerunMD && bNotLastFrame)) {
		...
		beta = 1.0;
                ...
                {	    
                    ...
                    d_x = state_global->x[at_id[i]][0];
                    d_y = state_global->x[at_id[i]][1];                    
                    d_z = state_global->x[at_id[i]][2];
		    
                    dL_x[i] = d_x;
                    dL_y[i] = d_y;
                    dL_z[i] = d_z;                    
                    
                    av_x[i][count1] = dL_x[i];
		    ...
		    if(diff_t < 1) diff_t += ((int)tau_0);
                    ...
                    sigma_x[i] = (av_x[i][count1]);
                    sigma_y[i] = (av_y[i][count1]);                    
                    sigma_z[i] = (av_z[i][count1]);
